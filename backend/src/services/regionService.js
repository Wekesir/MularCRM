...
async function listRegions({ includeInactive = true } = {}) {
  const sql = includeInactive
    ...
    : 'SELECT * FROM regions WHERE is_active = 1 ORDER BY name ASC';
  const [rows] = await pool.query(sql);
  return rows.map(normalizeRegion);
}

async function getRegionById(id) {
  const [rows] = await pool.query('SELECT * FROM regions WHERE id = ? LIMIT 1', [id]);
  return rows[0] ? normalizeRegion(rows[0]) : null;
}

async function getRegionByName(name) {
  const [rows] = await pool.query('SELECT * FROM regions WHERE name = ? LIMIT 1', [name]);
  return rows[0] ? normalizeRegion(rows[0]) : null;
}
...
async function createRegion(data) {
  ...
  const existing = await getRegionByName(name);
  if (existing) {
    const err = new Error(`Region "${name}" already exists`);
    err.code = 'DUPLICATE';
    throw err;
  }
...
async function updateRegion(id, data) {
  ...
  if (data.name !== undefined && String(data.name).trim() !== existing.name) {
    ...
    const clash = await getRegionByName(name);
  ...
  await pool.query(
    'UPDATE regions SET name = ?, code = ?, description = ?, is_active = ? WHERE id = ?',
    [
      name,
...
async function deleteRegion(id) {
  ...
  const [[usage]] = await pool.query(
    `SELECT COUNT(*) AS cnt FROM call_centers
     WHERE region_id = ? AND deleted_at IS NULL`,
    [id]
  );
  if (Number(usage?.cnt) > 0) {
    const err = new Error(
      'Cannot delete a region that is still assigned to call centers. Reassign those call centers first.'
    );
    err.code = 'IN_USE';
    throw err;
  }
